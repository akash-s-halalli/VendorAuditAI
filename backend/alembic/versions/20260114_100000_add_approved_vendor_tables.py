...
def upgrade():
    ...
    if "approved_ai_vendors" not in existing_tables:
        op.create_table(
            "approved_ai_vendors",
    ...
    if "approved_use_cases" not in existing_tables:
        op.create_table(
            "approved_use_cases",
            sa.Column("id", sa.String(36), primary_key=True),
    ...
    if "vendor_deployments" not in existing_tables:
        op.create_table(
            "vendor_deployments",
            sa.Column("id", sa.String(36), primary_key=True),
            sa.Column("approved_vendor_id", sa.String(36), sa.ForeignKey("approved_ai_vendors.id", ondelete="CASCADE"), nullable=False),
    ...
    if "ai_tool_requests" not in existing_tables:
        op.create_table(
            "ai_tool_requests",
            sa.Column("id", sa.String(36), primary_key=True),
            sa.Column("organization_id", sa.String(36), sa.ForeignKey("organizations.id"), nullable=False),
